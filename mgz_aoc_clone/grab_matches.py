...
def execute(minimum_elo, maximum_elo, output_folder, player_id, add_to_db):
    if player_id:
        ...
        try:
          ...
          # )
          matches = requests.get(
              f"https://aoe2.net/api/player/matches?game=aoe2de&profile_id={player_id}&count=100"
              # only grab 100 most recent games, more than that likely wont have replays
          )
          print(matches.url)
    ...
    for match in matches:
        ...
        # if match["leaderboard_id"] != 3 and match["leaderboard_id"] != 13:  # and
        if match["leaderboard_id"] not in [3, 13]:  # and
            continue
        ...
        for player in match["players"]:
            try:
                ...
                # https://aoe.ms/replay/?gameId=248023785&profileId=11406458
            except Exception as e:
                print(e)
                continue
