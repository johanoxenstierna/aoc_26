...
from mgz_aoc_clone.mgz.reference import get_consts, get_dataset
from mgz_aoc_clone.mgz.fast import Action as ActionEnum
from mgz_aoc_clone.mgz.fast.header import parse
from mgz_aoc_clone.mgz.model.definitions import *
from mgz_aoc_clone.mgz.model.inputs import Inputs
from mgz_aoc_clone.mgz.common.chat import parse_chat, Chat as ChatEnum
from mgz_aoc_clone.mgz.common.diplomacy import get_diplomacy_type
...
def parse_match(handle):
    ...
    players_jo = {}
    for p in copy.deepcopy(data['players']):
        p['name'] = p['name'].decode('utf-8')
        if p['name'] != 'Gaia':
            players_jo[p['name']] = p

    # Parse gaia objects  - jo: this seems to add a bunch of stuff to players
    gaia = [
        Object(
